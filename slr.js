...
function construct_query(ast) {
	...
	return [null, construct_query(ast[1][0])];
    } else if (ast[0] === 8) {
	return [ast[1][0]+"/"+(ast[1][2][1].length), construct_query(ast[1][2])];
    } else if (ast[0] === 9) {
	return construct_query(ast[1][0]).concat([construct_query(ast[1][2])]);
    } else if (ast[0] === 10) {
	return [construct_query(ast[1][0])];
...
function parse_query(tokens) {
    return construct_query(parse(tokens, QUERY));
}
...
function construct_clauses(ast) {
	...
	return [null, construct_clauses(ast[1][0])];
    } else if (ast[0] === 11) {
	return [ast[1][0]+"/"+(ast[1][2][1].length), construct_clauses(ast[1][2])];
    } else if (ast[0] === 12) {
	return construct_clauses(ast[1][0]).concat([construct_clauses(ast[1][2])]);
    } else if (ast[0] === 13) {
	return [construct_clauses(ast[1][0])];
...
function parse_clauses(tokens) {
    return construct_clauses(parse(tokens, CLAUSES));
}
